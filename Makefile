...
test-with-coverage:
	GO111MODULE=on go test -count=1 -coverprofile=tmp.cov --coverpkg $(COVERAGE_PACKAGES) -timeout 90s github.com/kopia/kopia/...

test-with-coverage-pkgonly:
	GO111MODULE=on go test -count=1 -coverprofile=tmp.cov -timeout 90s github.com/kopia/kopia/...
