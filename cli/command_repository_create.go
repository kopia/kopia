...
func (c *commandRepositoryCreate) setup(svc advancedAppServices, parent commandParent) {
	...
	cmd.Flag("block-hash", "Content hash algorithm.").Envar(svc.EnvName("KOPIA_CONTENT_HASH_ALGORITHM")).PlaceHolder("ALGO").Default(hashing.DefaultAlgorithm).EnumVar(&c.createBlockHashFormat, hashing.SupportedAlgorithms()...)
	cmd.Flag("encryption", "Content encryption algorithm.").Envar(svc.EnvName("KOPIA_CONTENT_ENCRYPTION_ALGORITHM")).PlaceHolder("ALGO").Default(encryption.DefaultAlgorithm).EnumVar(&c.createBlockEncryptionFormat, encryption.SupportedAlgorithms(false)...)
	...
	//nolint:lll
	cmd.Flag("format-block-key-derivation-algorithm", "Algorithm to derive the encryption key for the format block from the repository password").Envar(svc.EnvName("KOPIA_FORMAT_BLOCK_KEY_DERIVATION_ALGORITHM")).Default(format.DefaultKeyDerivationAlgorithm).EnumVar(&c.createBlockKeyDerivationAlgorithm, format.SupportedFormatBlobKeyDerivationAlgorithms()...)
