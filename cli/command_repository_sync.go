...
func (c *commandRepositorySyncTo) runSyncWithStorage(ctx context.Context, src blob.Reader, dst blob.Storage) error {
	...
	if err := src.ListBlobs(ctx, "", func(srcmd blob.Metadata) error {
		...
		srcBlobs++
		c.outputSyncProgress(fmt.Sprintf("  Found %v BLOBs (%v) in the source repository, %v (%v) to copy", srcBlobs, units.BytesStringBaseEnv(totalSrcSize), len(blobsToCopy), units.BytesStringBaseEnv(totalCopyBytes)))
	...
	log(ctx).Infof(
		...
		len(blobsToDelete), units.BytesStringBaseEnv(totalDeleteBytes),
		inSyncBlobs, units.BytesStringBaseEnv(inSyncBytes),
	)
...
func (c *commandRepositorySyncTo) listDestinationBlobs(ctx context.Context, dst blob.Storage) (map[blob.ID]blob.Metadata, error) {
	...
	if err := dst.ListBlobs(ctx, "", func(bm blob.Metadata) error {
		...
		c.outputSyncProgress(fmt.Sprintf("  Found %v BLOBs in the destination repository (%v)", len(dstMetadata), units.BytesStringBaseEnv(dstTotalBytes)))
		return nil
	}); err != nil {
			...
			for m := range copyCh {
				...
				c.outputSyncProgress(
					fmt.Sprintf("  Copied %v blobs (%v), Speed: %v, ETA: %v",
						numBlobs, units.BytesStringBaseEnv(bytesCopied), speed, eta))
