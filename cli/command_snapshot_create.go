...
func (c *commandSnapshotCreate) setup(svc appServices, parent commandParent) {
	...
	cmd.Flag("all", "Create snapshots for files or directories previously backed up by this user on this computer. Cannot be used when a source path is also given.").BoolVar(&c.snapshotCreateAll)
...
func (c *commandSnapshotCreate) run(ctx context.Context, rep repo.RepositoryWriter) error {
	...
	if c.snapshotCreateAll && len(sources) > 0 {
		return errors.New("cannot use --all with a source path given")
	}
