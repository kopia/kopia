...
func (c *commandServerUserAddSet) setup(svc appServices, parent commandParent, isNew bool) {
	...
	cmd.Flag("user-password-hashing-algorithm", "[Experimental] Password hashing algorithm").Envar(svc.EnvName("USER_PASSWORD_HASHING_ALGORITHM")).Hidden().Default(user.DefaultPasswordHashingAlgorithm).EnumVar(&c.userSetPasswordHashAlgorithm, user.PasswordHashingAlgorithms()...)
