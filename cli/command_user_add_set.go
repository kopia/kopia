...
func (c *commandServerUserAddSet) setup(svc appServices, parent commandParent, isNew bool) {
	...
	//nolint:lll
	cmd.Flag("user-password-hashing-algorithm", "[Experimental] Password hashing algorithm").Envar(svc.EnvName("KOPIA_USER_PASSWORD_HASHING_ALGORITHM")).Hidden().Default(user.DefaultPasswordHashingAlgorithm).EnumVar(&c.userSetPasswordHashAlgorithm, user.PasswordHashingAlgorithms()...)
