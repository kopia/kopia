...
func (c *storageAzureFlags) Setup(svc StorageProviderServices, cmd *kingpin.CmdClause) {
	...
	cmd.Flag("azure-use-cli-credential", "Use Azure CLI cached credential (reads `az login` cache, including `az login --identity`).").Envar(svc.EnvName("AZURE_USE_CLI_CREDENTIAL")).BoolVar(&c.azOptions.UseAzureCLICredential)
