...
func (c *storageAzureFlags) Setup(svc StorageProviderServices, cmd *kingpin.CmdClause) {
	...
	cmd.Flag("Use Azure CLI token cache for Azure AD authentication (reads credentials from `az login` / `az login --identity`). Ensure the identity has appropriate RBAC (e.g. Storage Blob Data Contributor).").Envar(svc.EnvName("AZURE_USE_CLI_CREDENTIAL")).BoolVar(&c.azOptions.UseAzureCLICredential)
