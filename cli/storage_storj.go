...
func (c *storageStorjFlags) Setup(svc StorageProviderServices, cmd *kingpin.CmdClause) {
	...
	cmd.Flag("access-name", "Config name associated with access (permissions etc.)").Required().StringVar(&c.storjOptions.AccessName)
	cmd.Flag("key-or-grant", "Access grant or API key (the latter requires satellite address and passphrase)").Required().StringVar(&c.storjOptions.KeyOrGrant)
...
func (c *storageStorjFlags) Connect(ctx context.Context, createBucket bool, formatVersion int) (blob.Storage, error) {
	return storj.New(context.Background(), &c.storjOptions, createBucket)
}
