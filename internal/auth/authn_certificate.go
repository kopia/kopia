...
func (c *clientCertificateAuthenticator) IsValid(
	...
	for _, cert := range tlsInfo.State.PeerCertificates {
		...
		if username == cert.Subject.CommonName {
			log(ctx).Debugf("Found matching client certificate with serial %q for user %q", cert.SerialNumber.String(), username)
			return true
		} else {
			log(ctx).Debugf("Client certificate common name %q does not correspond to the requested username %q",
				tlsInfo.State.PeerCertificates[0].Subject.CommonName, username)
		}
	}
...
func (c *clientCertificateAuthenticator) Refresh(ctx context.Context) error {
	return nil
}
