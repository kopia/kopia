...
func (e *Manager) refreshAttemptLocked(ctx context.Context) error {
	contentlog.Log(ctx, e.log, "refreshAttemptLocked")
...
func (e *Manager) WriteIndex(ctx context.Context, dataShards map[blob.ID]blob.Bytes) ([]blob.Metadata, error) {
	...
	for {
		contentlog.Log(ctx, e.log, "WriteIndex")
	...
	if cs.WriteEpoch >= writtenForEpoch+2 {
		contentlog.Log(ctx, e.log, "index-write-extremely-slow")

		if err = e.deletePartiallyWrittenShards(ctx, written); err != nil {
			contentlog.Log1(ctx, e.log, "index-write-extremely-slow-cleanup-failed", logparam.Error("error", err))
		}
	...
	contentlog.Log1(ctx, e.log, "index-write-success", logparam.Int("results", len(results)))
