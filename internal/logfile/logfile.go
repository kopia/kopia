...
func (c *loggingFlags) setup(cliApp *cli.App, app *kingpin.Application) {
	...
	app.Flag("log-level", "Console log level").Envar(cliApp.EnvName("KOPIA_CONSOLE_LOG_LEVEL")).Default("info").EnumVar(&c.logLevel, logLevels...)
	...
	app.Flag("file-log-level", "File log level").Envar(cliApp.EnvName("KOPIA_FILE_LOG_LEVEL")).Default("debug").EnumVar(&c.fileLogLevel, logLevels...)
