...
func (t *tokenBucketBasedThrottler) SetLimits(limits Limits) {
	t.readOps.SetLimit(limits.ReadsPerSecond * t.window.Seconds())
	t.writeOps.SetLimit(limits.WritesPerSecond * t.window.Seconds())
	t.listOps.SetLimit(limits.ListsPerSecond * t.window.Seconds())
	t.upload.SetLimit(limits.UploadBytesPerSecond * t.window.Seconds())
	t.download.SetLimit(limits.DownloadBytesPerSecond * t.window.Seconds())
}
...
func NewThrottler(limits Limits, window time.Duration, initialFillRatio float64) SettableThrottler {
