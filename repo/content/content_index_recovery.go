...
func (bm *WriteManager) RecoverIndexFromPackBlob(ctx context.Context, packFile blob.ID, packFileLength int64, commit bool) ([]Info, error) {
	...
	ndx, err := index.Open(localIndexBytes.Bytes().ToByteSlice(), nil, bm.format.Transformer().Overhead)
	if err != nil {
...
func (sm *SharedManager) appendPackFileIndexRecoveryData(pending index.Builder, output *gather.WriteBuffer) error {
	...
	if err := sm.format.Transformer().ToRepository(localIndex.Bytes(), localIndexIV, &encryptedLocalIndex); err != nil {
