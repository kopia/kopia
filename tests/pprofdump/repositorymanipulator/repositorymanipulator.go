...
// RepositoryManipulator provides a way to run a kopia command.
type RepositoryManipulator struct {
...
// NewRepositoryManipulator instantiates a new RepositoryManipulator and returns its pointer.
func NewRepositoryManipulator(baseDirPath, dataRepoPath string) (*RepositoryManipulator, error) {
...
func (bm *RepositoryManipulator) ConnectOrCreateRepo(dataRepoPath string) error {
...
func (bm *RepositoryManipulator) getFileWriter() bool {
	fw, err := fiofilewriter.New()
	if err != nil {
		if errors.Is(err, fio.ErrEnvNotSet) {
			log.Println("Skipping pprofdump tests because FIO environment is not set")
		} else {
...
func (bm *RepositoryManipulator) writeRandomFiles(ctx context.Context, fileSize, numFiles int) error {
	// create random data
...
func (bm *RepositoryManipulator) RestoreGivenOrRandomSnapshot(snapID, restoreDir string) (string, error) {
...
func (bm *RepositoryManipulator) SetUpSystemUnderTest() error {
	fileSize := 100
	numFiles := 100
...
// creates snapshots of the source directory.
func (bm *RepositoryManipulator) SetUpSystemWithOneSnapshot() (string, error) {
...
func (bm *RepositoryManipulator) GenerateRandomFiles(fileSize, numFiles int) error {
	ctx := context.Background()
...
func (bm *RepositoryManipulator) TakeSnapshot(dir string) (snapID, stdout string, err error) {
...
func (bm *RepositoryManipulator) DeleteSnapshot(snapshotID string) (string, error) {
	err := bm.ConnectOrCreateRepo(bm.DataRepoPath)
	if err != nil {
		return "", err
	}
