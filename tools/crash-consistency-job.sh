...
#
# 1. kopia_crash_consistency_dir
# 2. kopia_exe_dir
# 3. test_duration
# 4. test_timeout
# 5. test_repo_path_prefix

# Environment variables that modify the behavior of the crash consistency job execution
#
...
readonly kopia_crash_consistency_dir="${1?Specify directory with kopia crash consistency git repo}"
...
cat <<EOF
--- Job parameters ----
kopia_crash_consistency_dir: '${kopia_crash_consistency_dir}'
...
# Extract git metadata on the crash consistency repo, perform a crash consistency run
pushd "${kopia_crash_consistency_dir}"
...
make -C "${kopia_crash_consistency_dir}" \
    KOPIA_EXE="${kopia_exe}" \
    GO_TEST='go test' \
